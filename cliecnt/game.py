...
class game():
    def __init__(self) -> None:
        ...
        self.map = MapCreate(self.screen)
        ...
        self.me = player(self.screen,self.client.start_x,self.client.start_y,self.name,self.client,self.map)
...
#이벤트 확인 및 처리 함수
        ...
        for event in pygame.event.get():#이벤트 가져오기
            ...
            if event.type == pygame.KEYDOWN:#키 눌림?
                ...
                if event.key == pygame.K_f:
                    if self.me.player_dir == 5:
                        self.me.rec.x += 80
                    elif self.me.player_dir == -5:
                        self.me.rec.x -= 80
                if self.game_state == 2 and event.key == pygame.K_SPACE:# 재시작
                    self.game_state = 0
                    self.me.hp = 10
                    self.client.hp = 10
                    self.me.arrow_del = 0
                    self.me.rec.x = self.client.start_x#
                    self.me.rec.y = self.client.start_y#
            if self.name is not None:#//game1
                self.game_state = 1
                if self.client.name != "":
                    self.me.game_mode = 1
    ...
    def game_run(self):#무한 반복 문
        while self.isActive:
            ...
            if self.game_state == 0:#////이름
                self.input_name()       
                self.me.addr = self.client.addr  

            elif self.game_state == 1: #////플레이
                self.map.draw_block(self.client.block_pos)
                self.map.draw()
